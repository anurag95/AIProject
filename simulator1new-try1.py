...
class Player1:
	...
	def checktwo(self,x,y,z):

	def Heuristic(self, temp_board, temp_block, flag):
		value = 0
		for i in range(0,3):								#Counting basic x and o
			for j in range(0,3):
				if temp_block[i][j] == 'x':
					value += 1000
				elif temp_block[i][j] == 'o':
					value -=1000

		if(temp_block[0][0]!='-' && temp_block[0][0]!='d'):
			value += checkcorner(temp_block,0,0)				#Check for corners and other stuff
		if(temp_block[0][2]!='-' && temp_block[0][2]!='d'):
			value += checkcorner(temp_block,0,2)
		if(temp_block[2][0]!='-' && temp_block[2][0]!='d'):
			value += checkcorner(temp_block,2,0)
		if(temp_block[2][2]!='-' && temp_block[2][2]!='d'):
			value += checkcorner(temp_block,2,2)
		if(temp_block[1][1]!='-' && temp_block[1][1]!='d'):
			value += checkmiddle(temp_board,1,1)
		if(temp_block[0][1]!='-' && temp_block[0][1]!='d'):
			value += checknoncorner(temp_board,0,1)
		if(temp_block[1][0]!='-' && temp_block[1][0]!='d'):
			value += checknoncorner(temp_board,1,0)
		if(temp_block[1][2]!='-' && temp_block[1][2]!='d'):
			value += checknoncorner(temp_board,1,2)
		if(temp_block[2][1]!='-' && temp_block[2][1]!='d'):
			value += checknoncorner(temp_board,2,1)

		value += diagonal(temp_board)

		for i in range(0,3):
			value += checksame(temp_block[i][0],temp_block[i][1],temp_block[i][2])
		
		for i in range(0,3):
			value += checksame(temp_block[0][i],temp_block[1][i],temp_block[2][i])

		value += checksame(temp_block[0][0],temp_block[1][1],temp_block[0][0])
	...
	# We get all the empty cells in allowed blocks. If they're all full, we get all the empty cells in the entire board.
		cell = self.alpha_beta_pruning(temp_board, temp_block, old_move, -10000, 10000, turn, 0)
		cell = cell[0:2]
		cell = (cell[0],cell[1])
		if cell[0] == -1:
			cells = self.blocks(temp_board, temp_block, old_move)
