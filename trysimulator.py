...
class Player32:
	...
	def Heuristic(self, temp_board, temp_block, flag, depth):
		...
		#			value += self.weighted_status(mytempblock, 20)
					...
					for i in xrange(0, 3):
						value += self.checksame(mytempblock[i][0],mytempblock[i][1],mytempblock[i][2],30)

					for i in xrange(0, 3):
						value += self.checksame(mytempblock[0][i],mytempblock[1][i],mytempblock[2][i],30)

					value += self.checksame(mytempblock[0][0],mytempblock[1][1],mytempblock[2][2],40)
					value += self.checksame(mytempblock[2][0],mytempblock[1][1],mytempblock[0][2],40)
		if depth%2 == 1:
			if not flag:
				value = -value
		else:
	...
	def move(self,temp_board,temp_block,old_move,flag):
		...
		cell = self.monte(temp_board, temp_block, old_move, flag)
